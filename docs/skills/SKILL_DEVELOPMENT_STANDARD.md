...
## 7. powerAPI/PowerSkill 架构模式 (cloudpss_skills_v2)

### 7.1 架构概述

`cloudpss_skills_v2` 采用 powerAPI/PowerSkill 分层架构，将仿真引擎适配与业务逻辑解耦：
...
设计理念借鉴 Java AWT/Swing：
- **powerAPI**: 对应 Java AWT，重量级组件，直接对接仿真引擎
- **PowerSkill**: 对应 Java Swing，轻量级组件，提供引擎无关的API
...
from cloudpss_skills_v2.powerskill import PowerFlowAPI, APIFactory

class PowerFlowAnalysisSkill:
    """技能使用 PowerSkill API 层实现引擎无关的业务逻辑"""
...
from cloudpss_skills_v2.powerskill.apis import APIFactory
...
| 版本 | 日期 | 修改内容 |
|------|------|----------|
| 1.2.0 | 2026-04-17 | 重命名架构：AWT→powerAPI, Swing→PowerSkill |
